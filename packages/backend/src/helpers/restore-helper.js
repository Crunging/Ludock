...
const request = /** @type {import("./contracts.js").RestoreRequest} */ (
...
/** @param {...FileHandle} handles */
const sync = async (...handles) => {
  for (const handle of new Set(handles)) await handle.sync();
};
/** @param {Pick<Stats, 'dev' | 'ino'>} a @param {Pick<Stats, 'dev' | 'ino'>} b */
...
async function pinRoot() {
  ...
  for (const part of parts) {
    const next = await pin(link(current, part));
    await close(current);
    current = next;
  }
  return current;
...
async function exists(parent, name) {
  try {
    return await fsp.lstat(link(parent, name));
  } catch (error) {
...
/** @param {FileHandle} root */
async function pinStage(root, optional = false) {
  try {
    return await pin(link(root, request.stage));
  } catch (error) {
    if (
      optional &&
...
/** @param {FileHandle} parent @param {string} name @param {FileHandle} handle */
...
/** @param {FileHandle} source @param {FileHandle} destination @param {string} [omitted] */
async function moveContents(source, destination, omitted) {
  ...
  for (const name of names) {
    ...
    if (
      ...
      (!sourceInfo.isFile() || sourceInfo.nlink !== 1)
    )
      fail();
...
async function main() {
  if (
    ...
    !/^\.ludock-restore-[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(
      request.stage,
    )
  )
    fail();
  ...
  if (request.operation === "cleanup") {
    ...
    for (const name of await fsp.readdir(link(stage)))
      await removeEntry(stage, name);
    await sync(stage);
    await unchanged(root, request.stage, stage);
    await fsp.rmdir(link(root, request.stage));
    await sync(root);
    return { ok: true };
  }
...
main()
  ...
  .finally(async () => {
    await Promise.all(
      [...opened].map((handle) => handle.close().catch(() => {})),
    );
  });
