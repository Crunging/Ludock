...
test("application log filters support keyboard clearing and fit the viewport", { tag: "@responsive" }, async ({ app, page }, testInfo) => {
  await mockLogs(page, [
...
test("turning off follow preserves the reading position while polling and pause stops polling", { tag: "@responsive" }, async ({ app, page }) => {
...
test("reading position follows a retained row when the full log buffer evicts older entries", { tag: "@responsive" }, async ({ app, page }) => {
  await page.clock.install();
